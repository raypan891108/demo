...
//    public bookController(){}
    ...
    public ResponseEntity updateEmployee(@PathVariable Integer id, @RequestBody AddBookFromRequest updateBook, HttpServletRequest request) {
    ...
    @Transactional //保证延迟加载的属性能够被正确初始化
    @PutMapping("/AddBookVersion/{bookName}")
    @Operation(summary = "增加書籍版本及庫存", description = " Add the Book Version Data")
    public ResponseEntity AddBookVersion(@PathVariable("bookName") String bookName, @RequestBody BookVersionFromRequest bookVersionFromRequest, HttpServletRequest request) {

        return bks.AddBookVersion(bookName, bookVersionFromRequest, request);

    }

    @PutMapping("/UpdateBookVersion/{bookName}")
    @Operation(summary = "更新書籍庫存", description = "Update the Book Version Data")
    public ResponseEntity UpdateBookVersion(@PathVariable("bookName") String bookName, @RequestBody BookVersionFromRequest bookVersionFromRequest, HttpServletRequest request) {

        return bks.UpdateBookVersion(bookName, bookVersionFromRequest, request);
